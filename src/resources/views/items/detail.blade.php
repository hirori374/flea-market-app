    ...
    <div class="item-detail__container">
        ...
        <div class="item-detail__info">
            ...
            <div class="info__wrapper--marks">
                ...
                <div class="info__mark"><img src="{{ asset('img/comment_icon.png') }}" alt="" class="mark__icon--comment"><span class="mark__count">{{$commentsCount}}</span></div>
            </div>
            <form action="/purchase/{{$item['id']}}" method="get" class="purchase-form__button">
                @csrf
                <button class="purchase-form__button-submit">購入手続きへ</button>
            </form>
            <div class="info__wrapper">
                <h3 class="info__title">商品説明</h3>
                <div class="info__description"><p class="info__item">{{$item['description']}}</p></div>
            </div>
            <div class="info__wrapper">
                <h3 class="info__title">商品の情報</h3>
                <div class="info__category">
                    <p class="info__sub-title">カテゴリー</p>
                    <div class="info__sub-item">
                        @foreach ($item->categories as $category)
                        <p class="info__sub-item--category">{{$category['name']}}</p>
                        @endforeach
                    </div>
                </div>
                <div class="info__condition"><p class="info__sub-title">商品の状態</p><p class="info__sub-item">{{$item['condition']}}</p></div>
            </div>
            <div class="comment">
                <h3 class="comment__title">コメント({{$commentsCount}})</h3>
                <div class="comment__content">
                    @foreach ($comments as $comment)
                    <div class="user">
                        <p class="user__icon"><img src="{{ asset('storage/' . $comment->user->icon) }}" alt="アイコン"></p>
